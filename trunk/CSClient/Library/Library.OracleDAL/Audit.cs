...
namespace Library.SQLServerDAL
{
	...
	public partial class Audit:IAudit
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("F_ID", "T_Audit"); 
		}
		...
		public bool Exists(int F_ID)
		{
			...
			strSql.Append("select count(1) from T_Audit");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Library.Model.Audit model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into T_Audit(");
			strSql.Append("F_Name,F_Content,F_Money,F_Time,F_WF_StateID,F_UserID,F_IsDelete,F_Date,F_Type)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@F_Date", SqlDbType.Date,3),
			...
			parameters[8].Value = model.F_Type;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Library.Model.Audit model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update T_Audit set ");
		...
		{
            string sSQL = "update T_Audit set F_IsDelete=1 where F_ID=" + F_ID;
            int rows = DbHelperSQL.ExecuteSql(sSQL);
            if (rows > 0)
            {
                return true;
            }
            ...
            //StringBuilder strSql=new StringBuilder();
            //strSql.Append("delete from T_Audit ");
            //strSql.Append(" where F_ID=@F_ID");
            //SqlParameter[] parameters = {
            //        new SqlParameter("@F_ID", SqlDbType.Int,4)
            //};
		...
		public Library.Model.Audit GetModel(int F_ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 F_ID,F_Name,F_Content,F_Money,F_Time,F_WF_StateID,F_UserID,F_IsDelete,F_Date,F_Type from T_Audit ");
			strSql.Append(" where F_ID=@F_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@F_ID", SqlDbType.Int,4)
			};
			...
			Library.Model.Audit model=new Library.Model.Audit();
		...
		/// 得到一个对象实体
		/// </summary>
		public Library.Model.Audit DataRowToModel(DataRow row)
		{
			Library.Model.Audit model=new Library.Model.Audit();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select F_ID,F_Name,F_Content,F_Money,F_Time,F_WF_StateID,F_UserID,F_IsDelete,F_Date,F_Type ");
			strSql.Append(" FROM T_Audit ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" F_ID,F_Name,F_Content,F_Money,F_Time,F_WF_StateID,F_UserID,F_IsDelete,F_Date,F_Type ");
			strSql.Append(" FROM T_Audit ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from T_Audit T ");
