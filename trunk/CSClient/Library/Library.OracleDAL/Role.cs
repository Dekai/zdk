...
namespace Library.SQLServerDAL
{
	...
	public partial class Role:IRole
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("F_ID", "T_Role"); 
		}
		...
		public int Add(Library.Model.Role model)
		{
			...
			parameters[1].Value = model.F_Order;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Library.Model.Role model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		public bool Delete(int F_ID)
		{
			...
			if (rows > 0)
			{
                ...
                string sql = "delete  from T_Role_Right_Relation where F_RoleID not in (select F_ID from T_Role) or F_RightID not in (select F_ID from T_Right)";
                 ...
                 sql = "delete  from T_Role_Workflow where F_RoleID = " + F_ID;
                 DbHelperSQL.ExecuteSql(sql);
		...
		public bool DeleteList(string F_IDlist )
		{
			...
			if (rows > 0)
            {
                ...
                sql = "delete  from T_Role_Workflow where F_RoleID in ("+F_IDlist + ")";
                DbHelperSQL.ExecuteSql(sql); 
				return true;
			}
			else
		...
		/// 得到一个对象实体
		/// </summary>
		public Library.Model.Role GetModel(int F_ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 F_ID,F_Name,F_Order from T_Role ");
			strSql.Append(" where F_ID=@F_ID");
			SqlParameter[] parameters = {
					new SqlParameter("@F_ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Library.Model.Role DataRowToModel(DataRow row)
		{
			Library.Model.Role model=new Library.Model.Role();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select F_ID,F_Name,F_Order ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" F_ID,F_Name,F_Order ");
			strSql.Append(" FROM T_Role ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
