...
namespace Library.BLL
{
	...
	/// EquimentAttendace
	/// </summary>
	public partial class EquimentAttendaceService
	{
		private readonly IEquimentAttendace dal=DataAccess.CreateEquimentAttendace();
		public EquimentAttendaceService()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Library.Model.EquimentAttendace GetModelByCache(int F_ID)
		{
			
			string CacheKey = "EquimentAttendaceModel-" + F_ID;
			...
			return (Library.Model.EquimentAttendace)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Library.Model.EquimentAttendace> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Library.Model.EquimentAttendace> DataTableToList(DataTable dt)
		{
			List<Library.Model.EquimentAttendace> modelList = new List<Library.Model.EquimentAttendace>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Library.Model.EquimentAttendace model;
        ...
        {
            return dal.GetSummer(projectids, equipmentlist, startdate, enddate);
        }
	}
}
